...
class Cart
{
	...
	/*Ajna Studio work */
    public function get_sconted_val($ids, $sconto, $attribute= 'price'){
        $result = 0;
            ...
            foreach ($items as $item) {
                if (isset($item['attributes'][$attribute])) {
                    if (in_array($item['id'], $ids))
                        $result += $item['attributes'][$attribute] * $item['quantity'];
                }
            }
        }

        return ($result * $sconto)/100;
    }
    public function sconto_item($id, $sconto, $attribute= 'price'){
        foreach ($this->items as $items){
            foreach ($items as $item) {
                if ((isset($item['attributes'][$attribute]))&&(intval($item['id'])==intval($id))) {
                    return floatval(($item['attributes'][$attribute] * $item['quantity']*$sconto)/100);
                }
            }
        }
        return 0;
